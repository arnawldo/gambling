...
def test__die_probs_uniform_by_defualt__succeeds():
    die = Die()
    assert all([prob <= (1 / len(die.probs))for prob in die.probs])
...
def test__biased_die_sides__less_than_4__raises():
    with pytest.raises(NotEnoughSidesError):
        die = BiasedDie(1, sides=[1, 2], probs=[0.5, 0.5], cpmf=[0.5, 1])
